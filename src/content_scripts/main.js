...
function mainJira() {
  ...
  if (!document.querySelector('.has-burp-panel') && toolbar) {
    var specificationTicketNumber =
      document.querySelector('#key-val').innerText;
    var specificationAccount = document
      .querySelector('#customfield_12010-val > a')
      .innerText.replace('&', '%26');
    var specificationVersion = '';
    var licenseTableFirstRowColumns = document.querySelectorAll(
      '#customfield_14211-val table tbody tr td'
    );
    if (licenseTableFirstRowColumns && licenseTableFirstRowColumns.length) {
      specificationVersion =
        licenseTableFirstRowColumns[1].firstChild.innerText;
    }
    var specificationTemplateURL =
      'https://docs.google.com/document/d/1FmwVEEDnw7wNTJ8b4WI5SQe9E18fA5mUC2l3nf7qbns/copy';
    ...
    var epicNameFieldSelector = document.querySelector(
      '#customfield_11314-val'
    );
    var epicLinkFieldSelector = document.querySelector(
      '#customfield_11310-val'
    );
    var epicTicketNumber = document.querySelector('#key-val').innerText;
    var epicParentTicketNumber = epicLinkFieldSelector
      ? epicLinkFieldSelector.children[0].attributes.href.value.replace(
          '/browse/',
          ''
        )
      : null;
    var epicNumber = epicNameFieldSelector
      ? epicTicketNumber
      : epicLinkFieldSelector
      ? epicParentTicketNumber
      : null;
    var redirectorURL =
      'https://storage.googleapis.com/burp-redirector/index.html';
    ...
    if (!epicNumber) {
      newUl = document
        .querySelector('#opsbar-comment-issue_container')
        .cloneNode(true);
